...
mod tests {
    ...
    fn test_key_not_existing() {
        ...
        let expected_error_string: String =
            format!("There is no key char for u8 value \"{}\".", 17);
        assert_eq!(
            KeyMapError::KeyNotExisting(expected_error_string.clone()),
            error
        );
        assert_eq!(
            format!("KeyNotExisting({:?})", expected_error_string),
            format!("{:?}", error)
        );
